...
class ExponentialDistribution():
    ...
    '''
    def fit(self, X, T, regularization=1, learning_rate=.0000001, early_stopping=True, max_iter=35000, tol=.1):
        if len(X) == 0:
            return
        X = np.array(X)
        T = np.array(T)
...
def TestEstimationOnTrainingData():
    ...
    betas, likelihood_vals, gradient_norm_vals = dist.fit(X_train, T_train, learning_rate=1e-6, regularization=1, early_stopping=False, max_iter=1000000)
    iters = range(len(likelihood_vals))
    print "Final betas:",betas
    print "Final Likelihood:",likelihood_vals[-1]
    print "Final Gradient Norm:",gradient_norm_vals[-1]
    f = open("FinalBetasReg1_BOS.json", "w")
    json.dump(betas.tolist(), f)
    f.close()

    f = open("FinalLikelihoodAndGradReg1_BOS.json", "w")
    json.dump([likelihood_vals, gradient_norm_vals], f)
    f.close()

    f = open("FinalBetasReg1_BOS.json", "r")
    betas = json.load(f)
    f.close()
    dist.setBetas(betas)
...
def loadTrainingData():
    f = open("./training_data/X_BOS.txt", "r")
    X = []
    T = []
    for line in f:
        ...
        X.append(l)
    f.close()
    f = open("./training_data/T_BOS.txt", "r")
    for line in f:
        T.append(float(line))
    f.close()
